...
interface ICheckTokenValidityConfig {
    tokenKey: string;
    cacheKey: string;
    refreshThreshold: number;
    outputType: "boolean" | "detailed";
    // Add these fields to access cache file parameters
    clientId?: string;
    scope?: string;
    tokenUrl?: string;
}

export const checkTokenValidityNode = createNodeDescriptor({
    ...
    function: async ({ cognigy, config }: INodeFunctionBaseParams) => {
        ...
        const typedConfig = config as unknown as ICheckTokenValidityConfig;
        ...
        try {
            ...
            let cachedData = cognigy.input[cacheKey] || cognigy.context[cacheKey] as any;
            ...
            if (!cachedData || !cachedData.tokenExpiresAt) {
                ...
                // FIXED: Also set simplified flags for If node conditions
                api.addToContext("tokenIsValid", false, "simple");
                api.addToContext("tokenNeedsRefresh", true, "simple");
            ...
            // FIXED: Also set a simplified flag for If node conditions
            api.addToContext("tokenIsValid", isValid && !needsRefresh, "simple");
            api.addToContext("tokenNeedsRefresh", needsRefresh, "simple");
