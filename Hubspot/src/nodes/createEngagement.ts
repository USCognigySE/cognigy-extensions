...
export const createEngagementNode = createNodeDescriptor({
    ...
    function: (async ({ cognigy, config }: ICreateEngagementParams) => {
        ...
        try {
            ...
            if (contactIds) {
                const contactIdList = contactIds.split(',').map(id => id.trim()).filter(id => id);
                for (const contactId of contactIdList) {
                    try {
                        await client.crm.associations.v4.basicApi.create(
                            getObjectType(engagementType),
                            parseInt(engagementResponse.id),
                            "contacts",
                            parseInt(contactId),
                            [{ associationCategory: "HUBSPOT_DEFINED", associationTypeId: getAssociationTypeId(engagementType, "contact") }]
                        );
            ...
            if (companyIds) {
                const companyIdList = companyIds.split(',').map(id => id.trim()).filter(id => id);
                for (const companyId of companyIdList) {
                    try {
                        await client.crm.associations.v4.basicApi.create(
                            ...
                            "companies",
                            parseInt(companyId),
                            [{ associationCategory: "HUBSPOT_DEFINED", associationTypeId: getAssociationTypeId(engagementType, "company") }]
                        );
                    } catch (assocError: unknown) {
...
// Helper function to get object type for associations API
function getObjectType(engagementType: string): string {
    ...
    return objectTypeMap[engagementType] || "tasks";
}

// Helper function to get association type IDs
function getAssociationTypeId(engagementType: string, targetType: string): number {
    const associationMap: { [key: string]: { [key: string]: number } } = {
        call: { contact: 194, company: 182 },
        email: { contact: 198, company: 186 },
        meeting: { contact: 200, company: 188 },
        task: { contact: 204, company: 192 }
    };

    return associationMap[engagementType]?.[targetType] || 204; // Default to task-contact association
}