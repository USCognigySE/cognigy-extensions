...
export interface IGetArticleParams extends INodeFunctionBaseParams {
	config: {
		connection: {
			key: string;
			subdomain: string;
...
export const getArticleNode = createNodeDescriptor({
	type: "getArticle",
	defaultLabel: {
		default: "Get Article",
	...
	function: async ({ cognigy, config, childConfigs }: IGetArticleParams) => {
		const { api } = cognigy;
...
export const onArticleFound = createNodeDescriptor({
	type: "onArticleFound",
...
export const onArticleNotFound = createNodeDescriptor({
